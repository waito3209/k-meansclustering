...
import time
import random
def kmax(data:np.array,g,expode:int,viewintervial:int,initrangea,initrangeb,path = 'data',stoptime = 2):
    #([x1,x2,x3..])
    ...
    for e in range(expode):
        ...
        for nn in range(g):
            for i in range(no_data):
                temp = [cal(output[x], data[i]) for x in range(g)]
                data_class[i] = np.argmin(np.asarray(temp))
            normtabel = plot(data, data_class, g, show=False)
            nnt = np.asarray(normtabel[nn])
            for f in range(no_feature):
                output[nn][f] = 1/2 *(np.max(np.asarray(nnt[:, f]))+np.min(np.asarray(nnt[:, f])))
                #output[nn][f] = (np.median(np.asarray(nnt[:, f])))
    ...
    plt.ylim([initrangea, initrangeb])
    print(max(maxleng))
    plt.title(f"K-max clust max :{max(maxleng)}")
    ax = plt.gca()
    ax.set_aspect('equal', adjustable='box')
